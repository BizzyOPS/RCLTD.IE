...
const isDevelopment = process.env.NODE_ENV !== 'production' && (process.env.NODE_ENV === 'development' || !process.env.NODE_ENV);

// ==================== SECURITY MONITORING CONFIGURATION ====================

const SECURITY_CONFIG = {
    // Attack detection thresholds (adjusted for development vs production)
    thresholds: {
        requestsPerMinute: isDevelopment ? 500 : 100,
        suspiciousRequestsPerMinute: isDevelopment ? 50 : 10,
        failedAttemptsPerHour: 50,
        uniqueIpsPerHour: 1000,
        largePayloadThreshold: 1024 * 1024, // 1MB
        suspiciousUserAgents: isDevelopment ? 20 : 5,
        // Development-specific settings
        localhostExemption: isDevelopment,
        minRiskScoreForAlert: isDevelopment ? 75 : 30,
        developmentIps: ['127.0.0.1', '::1', 'localhost']
    },
    ...
    // Response actions (adjusted for development)
    responses: {
        autoBlock: false,    // Set to true for automatic IP blocking
        alertAdmins: !isDevelopment, // Reduce admin alerts in development
        logAllRequests: false, // Set to true for comprehensive logging
        rateLimitAggressive: !isDevelopment // Less aggressive in development
    },
...
class SecurityMonitor {
    constructor(config = SECURITY_CONFIG) {
        ...
        // File write queue to prevent concurrent write issues
        this.alertWriteQueue = [];
        this.alertWriteInProgress = false;
    ...
    /**
     * Check if IP should be exempted from strict monitoring (development)
     */
    isLocalhostOrDevelopmentIp(ip) {
        return this.config.thresholds.localhostExemption && 
               this.config.thresholds.developmentIps.includes(ip);
    }
    ...
    async handleSuspiciousActivity(req, threatAnalysis, requestId) {
        ...
        // Apply localhost exemption for development
        if (this.isLocalhostOrDevelopmentIp(ip)) {
            // Reduce alert severity for localhost traffic
            if (threatAnalysis.riskScore < this.config.thresholds.minRiskScoreForAlert) {
                return; // Skip alerting for low-risk localhost activity
            }
            // Reduce risk score for localhost
            threatAnalysis.riskScore = Math.min(threatAnalysis.riskScore * 0.5, 60);
        }
        ...
        // Save alert to file (queued for atomic writes)
        if (this.config.monitoring.logToFile) {
            this.queueAlertWrite(alert);
        }
    ...
    /**
     * Queue alert for atomic write (prevents concurrent write issues)
     */
    queueAlertWrite(alert) {
        this.alertWriteQueue.push(alert);
        
        // Process queue if not already processing
        if (!this.alertWriteInProgress) {
            setImmediate(() => this.processAlertQueue());
        }
    }

    /**
     * Process alert write queue atomically
     */
    async processAlertQueue() {
        if (this.alertWriteInProgress || this.alertWriteQueue.length === 0) {
            return;
        }
        
        this.alertWriteInProgress = true;
        
        try {
            // Process all queued alerts in batches by date
            const alertsByDate = new Map();
            
            // Group alerts by date
            while (this.alertWriteQueue.length > 0) {
                const alert = this.alertWriteQueue.shift();
            ...
            // Write each date's alerts atomically
            for (const [date, alerts] of alertsByDate) {
                await this.writeAlertsToFile(date, alerts);
            }
        } catch (error) {
            console.error('[SECURITY-MONITOR] Failed to process alert queue:', error);
        } finally {
            this.alertWriteInProgress = false;
            
            // Process any new alerts that arrived while processing
            if (this.alertWriteQueue.length > 0) {
                setImmediate(() => this.processAlertQueue());
            }
        }
    }
    ...
    /**
     * Atomically write alerts to file using append-only approach
     */
    async writeAlertsToFile(date, newAlerts) {
        try {
            const alertFile = path.join(this.config.paths.alertLogs, `alerts-${date}.json`);
            const tempFile = alertFile + '.tmp';
            ...
            if (await fs.exists(alertFile)) {
                try {
                    existingAlerts = await fs.readJSON(alertFile);
                    if (!Array.isArray(existingAlerts)) {
                        existingAlerts = [];
                    }
                } catch (parseError) {
                    // If file is corrupted, start fresh
                    console.warn(`[SECURITY-MONITOR] Corrupted alert file detected, starting fresh: ${alertFile}`);
                    existingAlerts = [];
                }
            }
            ...
            await fs.writeJSON(tempFile, allAlerts, { spaces: 2 });
            
            // Atomically replace the original file
            await fs.move(tempFile, alertFile);
            
        } catch (error) {
            console.error(`[SECURITY-MONITOR] Failed to write alerts to file for ${date}:`, error);
            
            // Clean up temp file if it exists
            const tempFile = path.join(this.config.paths.alertLogs, `alerts-${date}.json.tmp`);
            try {
    ...
    /**
     * Legacy method kept for compatibility (now uses queue)
     */
